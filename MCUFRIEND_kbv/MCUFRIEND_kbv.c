...
MCUFRIEND_MCUFRIEND_kbv():Adafruit_GFX(240, 320)
{
    // we can not access GPIO pins until AHB has been enabled.
}
#else
MCUFRIEND_MCUFRIEND_kbv(int CS, int RS, int WR, int RD, int RST):Adafruit_GFX(240, 320)
{
    // we can not access GPIO pins until AHB has been enabled.
}
#endif
...
void MCUFRIEND_WriteCmdData(uint16_t cmd, uint16_t dat)
{
    CS_ACTIVE;
    WriteCmd(cmd);
    WriteData(dat);
...
void MCUFRIEND_pushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
void MCUFRIEND_setRotation(uint8_t r)
{
...
void MCUFRIEND_drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void MCUFRIEND_fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t end;
#if defined(SUPPORT_9488_555)
    if (is555) color = color565_to_555(color);
...
void MCUFRIEND_pushColors(uint16_t * block, int16_t n, bool first)
{
    uint16_t color;
    CS_ACTIVE;
    if (first) {
...
void MCUFRIEND_pushColors(uint8_t * block, int16_t n, bool first)
{
    uint16_t color;
    uint8_t h, l;
    CS_ACTIVE;
...
void MCUFRIEND_pushColors(const uint8_t * block, int16_t n, bool first, bool bigend) //costs 28 bytes
{
    uint16_t color;
    uint8_t h, l;
    CS_ACTIVE;
...
void MCUFRIEND_invertDisplay(boolean i)
{
    uint8_t val;
...
void MCUFRIEND_begin(uint16_t ID)
{
